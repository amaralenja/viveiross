...
function FinanceiroPage() {
  ...
  async function pdf(){
    ...
    const doc=new jsPDF();doc.setFontSize(16);doc.text("Financeiro Pessoal",14,20);doc.setFontSize(9);doc.text(`Período: ${fMes||"Todos"} · ${fmtD(hoje())}`,14,27);
    ...
    autoTable(doc,{startY:56,head:[["Data","Tipo","Descrição","Categoria","Valor"]],body:toExport.map(l=>[fmtD(l.data),l.tipo==="receita"?"Receita":"Despesa",l.descricao,l.categoria,`${l.tipo==="receita"?"+":"-"}${brl(Number(l.valor))}`]),styles:{fontSize:8},headStyles:{fillColor:[30,41,59]}});
  ...
  return (
    <div className="space-y-4">
      ...
      {tab==="lancamentos"&&<>
        <div className="grid grid-cols-3 gap-2"><div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-3"><p className="text-[10px] uppercase text-emerald-600 font-bold">Receitas</p><p className="text-lg font-black text-emerald-600">{brl(tot.rec)}</p></div><div className="rounded-xl bg-rose-500/10 border border-rose-500/20 p-3"><p className="text-[10px] uppercase text-rose-600 font-bold">Despesas</p><p className="text-lg font-black text-rose-600">{brl(tot.desp)}</p></div><div className={`rounded-xl border p-3 ${tot.saldo>=0?"bg-muted/40":"bg-rose-500/5 border-rose-500/30"}`}><p className="text-[10px] uppercase text-muted-foreground font-bold">Saldo</p><p className={`text-lg font-black ${tot.saldo>=0?"text-foreground":"text-rose-600"}`}>{brl(tot.saldo)}</p></div></div>
        <div className="flex gap-2 flex-wrap items-center"><input type="month" value={fMes} onChange={e=>setFMes(e.target.value)} className="app-input h-8 w-auto text-xs"/><select value={fCat} onChange={e=>setFCat(e.target.value)} className="app-input h-8 w-auto text-xs"><option value="todas">Todas</option>{catsUnificadas.map(c=><option key={c} value={c}>{c}</option>)}</select><button onClick={pdf} className="h-8 px-2.5 rounded-lg bg-primary text-primary-foreground text-xs font-bold flex items-center gap-1"><FileDown className="size-3"/>PDF{sel.size>0?` (${sel.size})`:""}</button><button onClick={toggleAll} className="h-8 px-2 rounded-lg border text-xs font-semibold hover:bg-muted">{sel.size===filtrados.length&&filtrados.length>0?"Limpar":"Todos"}</button><div className="flex-1"/><button onClick={()=>{reset();setShowForm(true)}} className="h-8 px-3 rounded-lg bg-primary text-primary-foreground text-xs font-bold flex items-center gap-1"><Plus className="size-3.5"/>Novo</button></div>
        {showForm&&<form onSubmit={e=>{e.preventDefault();saveMut.mutate()}} className="rounded-xl bg-card border p-3 space-y-2"><div className="flex gap-1 p-0.5 bg-muted rounded-lg"><button type="button" onClick={()=>setTipo("despesa")} className={`flex-1 h-8 rounded-md font-bold text-xs ${tipo==="despesa"?"bg-rose-500 text-white":"text-muted-foreground"}`}><TrendingDown className="size-3 inline mr-0.5"/>Despesa</button><button type="button" onClick={()=>setTipo("receita")} className={`flex-1 h-8 rounded-md font-bold text-xs ${tipo==="receita"?"bg-emerald-600 text-white":"text-muted-foreground"}`}><TrendingUp className="size-3 inline mr-0.5"/>Receita</button></div><div className="grid grid-cols-2 gap-2"><input required value={desc} onChange={e=>setDesc(e.target.value)} placeholder="Descrição" className="app-input h-9 text-xs"/><input required value={val} onChange={e=>setVal(e.target.value.replace(/[^0-9.,]/g,""))} placeholder="Valor R$" className="app-input h-9 text-xs" inputMode="decimal"/></div><div className="grid grid-cols-2 gap-2"><select value={cat} onChange={e=>setCat(e.target.value)} className="app-input h-9 text-xs">{catsUnificadas.map(c=><option key={c} value={c}>{c}</option>)}</select><input type="date" value={dt} onChange={e=>setDt(e.target.value)} className="app-input h-9 text-xs"/></div><div className="flex gap-2"><button type="button" onClick={reset} className="flex-1 h-8 rounded-lg border text-xs font-semibold">Cancelar</button><button type="submit" disabled={saveMut.isPending} className="flex-1 h-8 rounded-lg bg-primary text-primary-foreground text-xs font-semibold">{saveMut.isPending?"Salvando...":"Salvar"}</button></div></form>}
        {filtrados.length===0?<div className="p-6 rounded-xl border-2 border-dashed text-center text-xs text-muted-foreground">Nenhum lançamento no período.</div>:<div className="space-y-1">{filtrados.map(l=><div key={l.id} className="flex flex-wrap sm:flex-nowrap items-center gap-1.5 p-2.5 rounded-lg bg-card border text-xs"><input type="checkbox" checked={sel.has(l.id)} onChange={()=>toggleSel(l.id)} className="size-3.5 shrink-0 mt-0.5"/><span className="text-muted-foreground w-16 shrink-0">{fmtD(l.data)}</span><span className={`font-bold w-28 shrink-0 text-right ${l.tipo==="receita"?"text-emerald-600":"text-rose-600"}`}>{l.tipo==="receita"?"+":"-"}{brl(Number(l.valor))}</span><span className="truncate flex-1 min-w-0 font-medium">{l.descricao}</span><span className="text-muted-foreground bg-muted px-1.5 py-0.5 rounded text-[10px] shrink-0">{l.categoria}</span><button onClick={()=>edit(l)} className="size-6 rounded hover:bg-muted flex items-center justify-center shrink-0"><Pencil className="size-3"/></button><button onClick={()=>{if(confirm("Apagar?"))delMut.mutate(l.id)}} className="size-6 rounded hover:bg-destructive/10 hover:text-destructive flex items-center justify-center shrink-0"><Trash2 className="size-3"/></button></div>)}</div>}
      </>}

      {tab==="relatorio"&&<>
        <div className="grid grid-cols-3 gap-2"><div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-3"><p className="text-[10px] uppercase text-emerald-600 font-bold">Receitas</p><p className="text-lg font-black text-emerald-600">{brl(tot.rec)}</p></div><div className="rounded-xl bg-rose-500/10 border border-rose-500/20 p-3"><p className="text-[10px] uppercase text-rose-600 font-bold">Despesas</p><p className="text-lg font-black text-rose-600">{brl(tot.desp)}</p></div><div className={`rounded-xl border p-3 ${tot.saldo>=0?"bg-muted/40":"bg-rose-500/5 border-rose-500/30"}`}><p className="text-[10px] uppercase text-muted-foreground font-bold">Saldo</p><p className={`text-lg font-black ${tot.saldo>=0?"text-foreground":"text-rose-600"}`}>{brl(tot.saldo)}</p></div></div>
        <div className="flex gap-2 flex-wrap"><input type="month" value={fMes} onChange={e=>setFMes(e.target.value)} className="app-input h-8 w-auto text-xs"/><button onClick={pdf} className="h-8 px-2.5 rounded-lg bg-primary text-primary-foreground text-xs font-bold flex items-center gap-1"><FileDown className="size-3"/>PDF</button></div>
        <div className="rounded-xl border bg-card p-4"><h3 className="font-bold text-sm mb-3 flex items-center gap-2"><PieChart className="size-4 text-primary"/>Gastos por Categoria</h3><div className="space-y-1.5">{Object.entries(tot.pc).sort((a,b)=>b[1]-a[1]).map(([c,v])=><div key={c} className="flex items-center gap-2 text-sm"><span className="w-32 truncate text-muted-foreground">{c}</span><div className="flex-1 bg-secondary h-2 rounded-full overflow-hidden"><div className="bg-primary h-full rounded-full" style={{width:`${tot.desp>0?Math.min(100,(v/tot.desp)*100):0}%`}}/></div><span className="w-24 text-right font-semibold">{brl(v)}</span></div>)}</div></div>
      </>}
